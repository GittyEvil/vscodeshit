...
def fight(enemy, ability):
    ...
    def attPower(weapon, ability, ultimate, damage, aDamage):
        ...
        if weapon == "Gammal rostig svärd":
            damage = random.randint(35,50)
        if weapon =="Träpinne":
            damage = random.randint(20,40)
        if weapon == "Gammal pilbåge":
        ...
        if ability == "SWORDSPIN":
            aDamage = 80
        if ultimate == "FATE SEALED":
            aDamage = 150
        if ability == "FIREBALL":
            aDamage = 70
        if ultimate == "PRIMORDIAL BURST":
            aDamage = 1000
        if ability == "QUICKSHOT":
            aDamage = 60
        if ultimate == "CROSSBOW":
            aDamage = 80

        return damage, aDamage

    def attack(enemy, health, eHealth):
        attPower(weapon, ability, ultimate, damage, aDamage)
        eHealth -= damage
